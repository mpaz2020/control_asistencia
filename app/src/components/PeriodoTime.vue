<template>
  ...
  >
    ...
    <template v-slot:append>
      <q-icon name="img:/icons/time-black.svg" class="cursor-pointer">
        <q-popup-proxy transition-show="scale" transition-hide="scale">
          <q-time
            :model-value="modelValue"
            format24h
...
const optionsFn = (date) => {
  ...
  if (props.previousPeriodo)
    return optionsFnMinDate(date, props.previousPeriodo);
...
function optionsFnTime(hr, min, sec) {
  if (!props.nextPeriodo && !props.previousPeriodo) return true;
  if (props.nextPeriodo)
    return optionsFnMaxTime(hr, min, sec, props.nextPeriodo, props.modelValue);
  if (props.previousPeriodo)
    return optionsFnMinTime(
      hr,
      min,
      sec,
      props.previousPeriodo,
      props.modelValue
    );
  return true;
}
...
function optionsFnMaxDate(date, nextDate) {
    return (
      date <=
      formatDate(extractDate(nextDate, `${FORMATO_FECHA} HH:mm`), "YYYY/MM/DD")
    );
  }
  ...
  function optionsFnMinDate(date, previousDate) {
    return (
      date >=
        formatDate(
          extractDate(previousDate, `${FORMATO_FECHA} HH:mm`),
          "YYYY/MM/DD"
